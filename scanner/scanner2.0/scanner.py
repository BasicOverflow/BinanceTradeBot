...
class Scanner(object): 
    ...
    def __init__(self,trading_interval="1m",host="localhost",port=12000,MAX_CLIENTS=350):
        self.trading_interval = trading_interval
        self.server_address = (host,port)
        self.MAX_CLIENTS = MAX_CLIENTS
        self.sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        #Activate array to hold websocket streams for sybmols from scanner clients
        #NOTE: object format for when appended to self.clients: 
        # {"request":request,   <--- request is a dictionary inside the dictionary 
        #  "tcp_conn":tcp_connection} <--- socket object
        self.clients = []


    
    def accept_incoming_requests(self):
        '''Receives incoming requests from scanner client and processes them'''
        ...
        while self.listening:
            ...
            try:
                ...
                #Process request:
                self.process_scanner_request(request,conn)
            except IndexError:
    ...
    def process_scanner_request(self,request,tcp_conn):
        '''unpacks given request and adds it to self.clients to init a websocket stream'''
        #If the position is long, then its assumed that the incoming client wants to look for an entry position
        #into the market. If the position is sell, then its assumed that the client has bought into the market and is
        #now looking for an exit position out of the market.
        position = request["position"]
        if position == "long":
            #If the client is a long position, then append it to self.clients using the format laid out in __init__
            client = {
                "request": request,
                "tcp_conn": tcp_conn
            }
            ...
            #If the client is a sell position, then things get a bit more complicated:
            # since the client is looking to sell, then that means there is already a long client that exists with the same symbol.
            # Therefore, that long client would need to be replaced with the sell client while also switching the tcp connection from the 
            # original long client to the new sell client.
            
            #Locate the old long client:
            old_client = [client for client in self.clients if i["request"]["symbol"] == request["request"]["symbol"]][0]
            print(f"Old client: {str(old_client)}")
